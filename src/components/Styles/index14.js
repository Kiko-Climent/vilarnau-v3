...
export default function StyleSliderMobile3New({ ready }) {
  const sliderRef       = useRef(null);
  const counterRef      = useRef(null);
  const previewsRef     = useRef(null);
  const sliderImagesRef = useRef(null);
  const textRef         = useRef(null);
  const innerRef        = useRef(null); // dentro del pl-4 para medir el left real del contenido
  ...
  // Ancho de la foto: desde el borde del contenido (pl-4) hasta el inicio de "about"
  ...
  return (
    ...
    >
      ...
      {/* ── Slider + contador — pl-4 solo izquierda ──────────────────────── */}
      <div ref={innerRef} className="flex flex-row flex-1 gap-2 items-end pl-4">

        {/* Foto — desde pl-4 hasta la "a" de about */}
        <div
        ...
        {/* Contador — ocupa el espacio entre "about" y el borde derecho */}
      ...
      {/* ── Info — pl-4 alineado con la foto ─────────────────────────────── */}
      <div
        ref={textRef}
