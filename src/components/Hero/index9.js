...
export default function Hero9({scrollYProgress}) {
  ...
  return (
    <div className="gallery relative w-full h-screen overflow-hidden bg-white" ref={galleryRef}>
      {imageFilenames.map((filename, index) => (
        <div className="img opacity-0 scale-0" key={index}>
      ...
      <p
        ref={text1Ref}
        className="absolute text-4xl md:text-5xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white mix-blend-difference font-medium z-50 opacity-0"
      >
        salon vilarnau
      </p>
      ...
      <p
        ref={text3Ref}
        className="absolute text-4xl md:text-5xl blur-[0.5px] top-1/2 left-1/2 -translate-x-1/2 text-white mix-blend-difference font-medium z-50 opacity-0"
      >
        info@vilarnau.de
      </p>
